...
function getMentorshipUrl() {
  const mentorship = WIS_DATA.mentorship;
  return mentorship && mentorship.url ? mentorship.url : null;
}

function getContactEmail() {
  return WIS_DATA.links.email;
}
...
function renderPerson(m, showPhotoPlaceholder) {
  let photoContent;
  if (m.image) {
    photoContent = `<img src="${m.image}" alt="${m.name}" class="team-photo">`;
  } else if (showPhotoPlaceholder) {
    photoContent = `<span class="photo-soon" aria-hidden="true">${m.initials}</span><span class="photo-soon-label">Photo coming soon</span>`;
  } else {
    photoContent = `<span aria-hidden="true">${m.initials}</span>`;
  }
...
function initContactForm() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (note) {
      note.textContent = `This form is a UI placeholder and does not submit yet. Please email ${email} instead.`;
    }
  });
}
